...
def birthday_attack(hash_func, hash_length: int, attackAttempts: int, algorithm: str, type: str):
    ...
    for _ in range(attackAttempts):
        ...
        if hashed_value in hash_dict and hash_dict[hashed_value] != message:
            print(f"Collision found for {algorithm} after {_} attempts!")
...
def pollard_rho(type: str, hash_func, algorithm: str, compare_length: int, attackAttempts):
    ...
    for i in range(attackAttempts):
        ...
        if x_hash[:compare_length] == y_hash2[:compare_length]:
            ...
            print(f"Hash Value: {x_hash[:compare_length]}")
...
def second_preimage_attack(hash_func, target_hash, char_type, max_length, attempts: int, algorithm: str, type: str, compare_length=int):
    ...
    target_hash_prefix = target_hash[:compare_length]
    ...
    for _ in range(attempts):
        ...
        if hash_value[:compare_length] == target_hash_prefix: 
            print(f"second_Preimage found after {_} attempts!\nMessage is {candidate_str} \nHash is {hash_value[:compare_length]} matching target hash {target_hash[:compare_length]})")
...
# 暴力枚举破解函数，用于原象攻击
def brute_force_hash(hash_func, target_hash, char_type, max_length, compare_length=int):
    """
    对给定的哈希值进行暴力破解以寻找原象
    ...
    for length in range(1, max_length + 1):
        ...
        for guess in itertools.product(charset, repeat=length):
            ...
            # 如果哈希值匹配，则返回原象
            if guess_hash[:compare_length] == target_hash[:compare_length]:
              print(f"Message is: {guess_str}")
              return True
    return None
...
if attack_type == 'birthday':
    ...
    print(f"Target string: {target_string}, Target hash: {target_hash[:t_compare_length]}")
    print(f"Attempting second_preimage attack on {hash_algorithm} with target hash {target_hash[:t_compare_length]}...")
    second_preimage_attack(hash_message, target_hash, char_type, max_length, attackAttempts, hash_algorithm, type=char_type, compare_length=t_compare_length)
    ...
    target_string = random_string(max_length, char_type)
    # print(f"Message 1: {target_string}")
    target_hash = hash_message(target_string, hash_algorithm)
    print(f"Hash Value: {target_hash[:compare_length]}")
    # 调用暴力枚举破解函数，根据哈希值生成算法使用不同的哈希算法暴力破解原象
    if hash_algorithm == 'MD5':
        result = brute_force_hash(hashlib.md5, target_hash, char_type, max_length, compare_length=compare_length)
    elif hash_algorithm == 'SHA1':
        result = brute_force_hash(hashlib.sha1, target_hash, char_type, max_length, compare_length=compare_length)
    elif hash_algorithm == 'SHA256':
        result = brute_force_hash(hashlib.sha256, target_hash, char_type, max_length, compare_length=compare_length)
    #打印结果
    if result:
        print(f"Brute Force Preimage Attack Successfully!")
    else:
